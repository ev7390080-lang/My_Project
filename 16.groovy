...
private void createInvoiceExtractDocument(String DOLLAR, Account account, Person_Id bgPerId, Person_Id parentPerId, String industry) {
  ...
  if(!isNull(bgPerId.getEntity().getEffectiveCharacteristic(getBillToCharType()))){
    ...
    if(!isEmptyOrNull(perCharType.getAdhocCharacteristicValue())){
     customerAddress1 = "Client Name" + ": " + perCharType.getAdhocCharacteristicValue();
    }
  } else {
    ///customerAddress1 = parentPerson.getPersonPrimaryName();
	 customerAddress1 = "Client Name" + ": " + parentPerson.getPersonPrimaryName();
  }
...
if ("ACCOUNT".equals(billLevel)) {
    billLevelValue = deriveBillLevelValue(billLevel, bgPerId);
    String billLevelValueCar = deriveBillLevelValue("CARRIER", bgPerId);
    customerAddress2 = "CARRIER: " + checkStringNull(billLevelValueCar.trim());
    customerAddress3 = "ACCOUNT: " + checkStringNull(billLevelValue.trim());
} else if ("GROUP".equals(billLevel)) {
    billLevelValue = deriveBillLevelValue(billLevel, bgPerId);
    String billLevelValueCar = deriveBillLevelValue("CARRIER", bgPerId);
    String billLevelValueAccount = deriveBillLevelValue("ACCOUNT", bgPerId);

    customerAddress2 = "CARRIER: " + checkStringNull(billLevelValueCar.trim());
    customerAddress3 = "ACCOUNT: " + checkStringNull(billLevelValueAccount.trim());
    customerAddress4 = "GROUP: " + checkStringNull(billLevelValue.trim());
} else if ("CARRIER".equals(billLevel)) {
    billLevelValue = deriveBillLevelValue(billLevel, bgPerId);
    customerAddress2 = "CARRIER: " + checkStringNull(billLevelValue.trim());
}
...
customerAddress3Elem.setTextContent(customerAddress3.trim());
customerAddress.appendChild(customerAddress3Elem);

Element customerAddress4Elem = doc.createElement(ADDRESS4);
customerAddress4Elem.setTextContent(customerAddress4.trim());
customerAddress.appendChild(customerAddress4Elem);

String customerAddress5 = billToAddress1 + (!isEmptyOrNull(billToAddress2) ? (" " + billToAddress2) : "");
Element customerAddress5Elem = doc.createElement(ADDRESS5);
customerAddress5Elem.setTextContent(customerAddress5);
customerAddress.appendChild(customerAddress5Elem);

String customerAddress6 = billToCity + (!isEmptyOrNull(billToState) ? (", " + billToState.trim()) : "") + (!isEmptyOrNull(billToZip) ? (" " + billToZip.trim()) : "");
Element customerAddress6Elem = doc.createElement(ADDRESS6);
customerAddress6Elem.setTextContent(customerAddress6);
customerAddress.appendChild(customerAddress6Elem);
